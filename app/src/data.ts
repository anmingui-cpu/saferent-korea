...
// ─── 사진 분류 ───
export const REQUIRED_PHOTO_CATS = ['거실', '방', '주방', '화장실'] as const
...
export const DOC_TYPE_DESCS: Record<string, { why: string; role: string }> = {
  '등기부등본': {
    ...
    role: '건물 소유자·근저당·압류·신탁 등 법적 상태를 전부 담고 있어. 인터넷등기소(iros.go.kr)에서 700원이면 바로 뗄 수 있어. 계약 당일에도 반드시 최신본을 다시 떼서 확인해.',
  },
  '건축물대장': {
    why: '불법 건축·용도 위반 여부를 확인하는 서류',
    role: '건물이 합법적으로 지어졌는지, 실제 면적·구조가 등기부와 일치하는지 체크해. 정부24(gov.kr)에서 무료로 발급받을 수 있어.',
  },
  '계약서': {
    why: '모든 약속이 법적으로 남는 유일한 증거',
    role: '보증금·월세·관리비·계약기간·특약 등 모든 조건을 담는 문서야. 구두 약속은 나중에 증거가 안 돼, 중요한 내용은 전부 특약에 넣어둬. 원본을 반드시 받아 보관해야 해.',
  },
  '중개대상물 확인설명서': {
    why: '중개사가 집 상태를 공식으로 설명하는 의무 서류',
    role: '건물의 권리관계·관리비·옵션·결함 등을 중개사가 직접 확인하고 서명해서 작성해. 분쟁이 생겼을 때 중개사 책임의 핵심 근거가 돼.',
  },
  '공인중개사 등록증': {
    why: '중개사가 정식 면허를 가진 사람인지 확인하는 증거',
    role: '정식 등록 중개사를 통해야만 중개사고 발생 시 공제보험으로 피해 보상을 받을 수 있어. 사무실 벽에 걸린 등록증·자격증·공제증서를 사진 찍어둬.',
  },
  '신분증 확인자료': {
    why: '계약 상대방이 실제 소유자인지 대조하는 증거',
    role: '등기부등본 소유자 이름과 계약자의 신분증이 같은 사람인지 직접 확인하는 과정이야. 동의 후 신분증 앞면 사진을 찍어두면 분쟁 시 증거가 돼.',
  },
  '영수증': {
    why: '돈을 줬다는 법적 증거로 반드시 받아야 해',
    role: '계약금·잔금 등 돈을 낼 때마다 영수증을 받거나 계좌이체 확인증을 캡처해 보관해. 현금으로 낸 경우엔 영수증이 유일한 증거가 돼.',
  },
  '보증보험 자료': {
    why: '보증금을 국가가 대신 지켜주는 보험 관련 서류',
    role: '주택도시보증공사(HUG)나 SGI서울보증의 전세보증보험에 가입하면 집주인이 보증금을 안 돌려줘도 보험사가 대신 지급해줘. 가입 조건과 서류를 여기 보관해.',
  },
  '기타': {
    why: '계약 관련 그 밖의 참고 자료 보관용',
    role: '위 항목에 해당하지 않는 메모·사진·서류 등을 모아두는 공간이야. 나중에 찾기 쉽도록 파일명을 구체적으로 붙여둬.',
  },
}
...
export const CHECKLIST_GROUPS: ChecklistGroup[] = [
  {
    name: '실내',
    items: [
      c('mold', '곰팡이', true, '벽 모서리랑 창틀, 옷장 뒤쪽을 꼭 들여다봐. 최근에 도배한 집이면 곰팡이를 가려놨을 수 있으니 퀴퀴한 냄새가 나는지도 맡아봐야 해'),
      c('leak', '누수', true, '천장이랑 벽에 얼룩이 있으면 누수 흔적이야. 화장실은 아랫집에 물 샌 적 없는지 중개사한테 꼭 물어보자'),
      c('condensation', '결로', true, '창틀 주변에 곰팡이나 물자국이 있는지 봐. 결로 생기는 집은 곰팡이로 바로 이어져'),
      c('waterpressure', '수압', true, '샤워기랑 세면대를 동시에 틀어봐. 부끄러워하지 말고 꼭 틀어봐야 해'),
      c('hotwater', '온수', true, '온수가 나올 때까지 얼마나 걸리는지, 온도는 충분한지 직접 확인해보자'),
      c('lock', '잠금장치', true, '현관 도어락이랑 창문 잠금장치 상태를 봐야 해. 낮은 층이면 방범창이 있는지도 꼭 확인해'),
      c('sewersmell', '하수구 냄새', true, '화장실이랑 싱크대 배수구에 코를 대봐. 냄새 나는 집은 살면서 계속 고생해'),
      c('smell', '악취'),
      c('ventilation', '환기'),
      c('sunlight', '채광'),
      c('noise', '소음'),
  ...
  },
  {
    name: '옵션',
    items: [
      c('aircon', '에어컨', true, '켜서 찬바람 나오는지 직접 확인하고, 연식이랑 마지막 청소가 언제였는지 물어봐'),
      c('boilerfridge', '냉장고'),
      c('washer', '세탁기'),
      c('induction', '인덕션'),
      c('gasstove', '가스레인지'),
  ...
  },
  {
    name: '건물',
    items: [
      c('cctv', 'CCTV', true, '공동현관, 복도, 주차장에 CCTV가 있는지 봐둬. 안전이랑 직결돼'),
  ...
  },
  {
    ...
    items: [
      ...
      c('construction', '공사장', true, '근처에 공사장이 있으면 언제 끝나는지 물어봐. 오래 걸리는 공사면 사는 내내 시달려'),
      c('odorfacility', '악취시설'),
    ],
  },
]
...
export const LEGAL_GROUPS: LegalGroup[] = [
  {
    name: '등기부등본',
    intro: '등기부등본은 이 집의 신분증이야. 인터넷등기소(iros.go.kr)에서 700원이면 바로 뗄 수 있어. 계약하는 날에도 최신본을 다시 떼서 확인하는 거, 잊지 말자.',
    items: [
      { id: 'reg_address', label: '주소 일치', desc: '등기부에 적힌 주소랑 계약서, 실제 건물 주소가 완전히 같은지 봐. 동·호수까지 하나하나 맞춰봐야 해' },
      { id: 'reg_owner', label: '소유자 확인', desc: '등기부 갑구에 있는 소유자 이름이랑 계약하러 나온 집주인의 신분증이 같은 사람인지 꼭 대조해보자' },
      { id: 'reg_joint', label: '공동명의', desc: '소유자가 두 명 이상이면 전원의 동의(위임장·인감증명서)가 있어야 해. 한 명하고만 계약하면 나중에 무효가 될 수 있어' },
      { id: 'reg_trust', label: '신탁 여부', desc: '갑구에 "신탁"이라고 적혀 있으면 집 소유권이 신탁회사에 있다는 뜻이야. 신탁회사 동의 없이 계약하면 보증금을 통째로 잃을 수 있으니 정말 조심해' },
      { id: 'reg_mortgage', label: '근저당', desc: '을구의 근저당 채권최고액에 네 보증금을 더한 금액이 집값의 70%를 넘으면 위험해. 집이 경매로 넘어가면 보증금을 못 돌려받을 수 있거든' },
      { id: 'reg_seizure', label: '압류', desc: '갑구에 압류가 있으면 집주인이 세금을 체납했다는 신호야. 이런 집은 계약하지 않는 게 좋아' },
      { id: 'reg_provisional', label: '가압류', desc: '가압류는 집주인이 소송에 걸려 있다는 뜻이야. 재판 결과에 따라 집이 경매로 넘어갈 수도 있어' },
      { id: 'reg_auction', label: '경매', desc: '경매개시결정 등기가 보이면 뒤도 돌아보지 말고 나와. 절대 계약하면 안 되는 집이야' },
      { id: 'reg_leaseright', label: '임차권등기', desc: '이전 세입자가 보증금을 못 받고 나갔다는 표시야. 집주인이 보증금 돌려줄 능력이 없다는 뜻이니 심각하게 봐야 해' },
    ],
  },
  {
    name: '공인중개사',
    intro: '중개사고가 나면 공제(보험)로 보호받을 수 있는데, 그건 정식 등록된 중개사와 거래했을 때 얘기야. 등록번호는 국가공간정보포털에서 조회할 수 있어.',
    items: [
      { id: 'ag_regno', label: '등록번호 조회', desc: '사무실에 걸려 있는 등록번호를 국가공간정보포털(nsdi.go.kr)이나 구청에 조회해서 진짜 등록된 중개사인지 확인해봐' },
      { id: 'ag_cert', label: '등록증 게시', desc: '사무실 벽에 중개사무소 등록증, 공인중개사 자격증, 공제증서가 걸려 있는지 둘러보자. 정상적인 사무실이면 다 걸려 있어야 해' },
      { id: 'ag_rep', label: '대표자 일치', desc: '등록증의 대표자와 실제로 계약을 진행하는 사람이 같은지 봐. 다른 사람이면 소속공인중개사가 맞는지 물어봐야 해' },
      { id: 'ag_office', label: '사무실 실재', desc: '계약은 등록증에 적힌 주소의 사무실에서 해야 해. 카페 같은 데서 계약하자고 하면 일단 의심해' },
      { id: 'ag_card', label: '명함 확인', desc: '명함에 적힌 상호랑 이름이 등록증과 같은지 확인해두자' },
    ],
  },
  {
    name: '계약서',
    intro: '말로 한 약속은 나중에 아무 소용이 없어. 모든 숫자와 조건은 반드시 계약서에 글자로 남겨야 해. 특약사항은 아끼지 말고 적극 활용해.',
    items: [
      { id: 'ct_address', label: '주소', desc: '계약서의 주소가 등기부등본과 완전히 같은지 봐. 지번, 동, 호수까지 전부' },
      ...
      { id: 'ct_fee', label: '관리비', desc: '관리비가 얼마인지, 수도·인터넷·청소비 중 뭐가 포함인지 명확히 적어. 나중에 딴소리 못 하게' },
      { id: 'ct_period', label: '계약기간', desc: '시작일과 종료일을 적어둬. 참고로 2년 미만으로 계약해도 세입자는 법으로 2년까지 살 권리가 있어' },
      { id: 'ct_downpay', label: '계약금', desc: '보통 보증금의 10%야. 반드시 소유자 명의 계좌로 이체하고 영수증을 받아둬' },
      { id: 'ct_balance', label: '잔금', desc: '잔금 치르는 날 등기부등본을 다시 떼봐. 그 사이에 새 근저당이 잡혔을 수도 있어. 확인하고 나서 이체하는 거야' },
      { id: 'ct_movein', label: '입주일', desc: '보통 입주일이 잔금일이야. 입주하면 그날 바로 전입신고하고 확정일자 받아. 이게 보증금 지키는 제일 중요한 절차야' },
      { id: 'ct_special', label: '특약', desc: '"잔금일까지 현재 등기 상태 유지", "보증보험 가입 협조" 같은 특약을 꼭 넣자. 정당한 요구니까 망설일 필요 없어' },
      { id: 'ct_cancel', label: '중도해지', desc: '중간에 이사 가게 되면 위약금이 있는지, 새 세입자 중개보수는 누가 내는지 미리 정해둬' },
      { id: 'ct_restore', label: '원상복구', desc: '나갈 때 어디까지 원래대로 해놔야 하는지 구체적으로 적어두자. 입주 전에 찍은 사진이 나중에 증거가 돼' },
...
export const RISK_ITEMS: RiskItem[] = [
  ...
  { id: 'r_seizure', label: '압류', score: 50, desc: '등기부에 압류가 있는 집이야. 집주인 재정에 문제가 있다는 뜻이야' },
  ...
  { id: 'r_cash_only', label: '현금만 요구', score: 40, desc: '계좌이체를 거부하고 현금만 달라는 건 증거를 안 남기려는 거야. 조심해' },
  { id: 'r_rush', label: '계약 재촉', score: 30, desc: '"오늘 아니면 나간다"면서 재촉하면 일단 의심해. 좋은 집은 하루 이틀 생각할 시간을 줘도 안 도망가' },
  ...
  { id: 'r_edit_contract', label: '계약서 수정 요구', score: 30, desc: '계약서를 쓴 다음에 고치자고 하거나 사본을 안 주면 문제가 있는 거야' },
